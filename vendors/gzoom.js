...
jQuery.fn.gzoom = function(settings) {
  ...
  this.each(function(){
    ...
    $slider = jQuery('<div class="gzoomSlider"></div>').insertAfter($div).css({ width:(settings.sW-42)+'px'});
    var $minus = jQuery('<div class="ui-icon ui-icon-circle-minus gzoombutton">&nbsp;</div>').insertAfter($div);

    $plus.click(function() {
      valore = parseInt($slider.slider('value'), 10) + settings.step;
  ...
  function setSliderValue(zoom) {
    $slider.slider('value', zoom);
  };

  return {
    setZoom : function(zoom) { setSliderValue(zoom) }
  };
